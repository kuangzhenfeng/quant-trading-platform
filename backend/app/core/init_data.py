...
async def init_users_from_env():
    """从环境变量初始化默认用户"""
    users = await list_users()
    if not users:
        try:
            await create_user(settings.AUTH_DEFAULT_USERNAME, settings.AUTH_DEFAULT_PASSWORD)
            print(f"[INIT] 创建默认用户: {settings.AUTH_DEFAULT_USERNAME}")
        except ValueError:
            pass


async def init_accounts_from_env():
    """从环境变量初始化账户配置"""
    ...
    if okx_config.get("api_key"):
        await account_service.add_account(BrokerConfig(
            id=f"okx_{mode}",
            broker="okx",
...
async def init_data_from_env():
    """从环境变量初始化所有数据"""
    await init_users_from_env()
    await init_accounts_from_env()
